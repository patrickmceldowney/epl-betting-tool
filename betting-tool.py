...
def create_lineup_dictionary(raw_data) -> Dict[int, str]:
    """
    Pull the lineup data and put it into an un-formatted dictionary

    :param raw_data: Extracts of HTML from Fantasy Football Scout that contain the lineup information.
    :return: A dictionary of the format {0: [player1, player2, ... player11], ... 20: [...] }
    """
    logger.info('--- Creating lineup dictionary (un-formatted) ----')
    lineup_dictionary = {}
    key = 0

    for team in raw_data:
        lineup_dictionary[key] = list(filter(None, team.text.splitlines()[1:]))
        logger.info(f'lineup {key}: {lineup_dictionary[key]}')
        assert len(lineup_dictionary[key]) == 11
        key += 1

    logger.info(f'Final un-formatted lineups: {lineup_dictionary}')
    assert len(lineup_dictionary) == 20

    logger.info('---- Created lineup dictionary (un-formatted) successfully ---- \n')
    return lineup_dictionary


def create_team_mapper(raw_data) -> Dict[int, str]:
    """
    Create a dictionary which maps the un-formatted lineup data to their respective team names.
    ...
    for team in range(2, 22):
        team_map[team - 2] = raw_data[team].text.split('Next')[0].replace('\n', '')
...
def create_final_dictionary(lineups_data: Dict[int, str], mapping_dictionary: Dict[int, str]) -> Dict[str, str]:
    """
    Uses the mapping dictionary to assign the team names to each predicted lineup.

    :param lineups_data: A dictionary with keys that match `mapping_dictionary` to obtain the respective team names.
    :param mapping_dictionary: A dictionary with keys that match the `lineups_data` to map team names to these value
...
def pull_fantasyfootballscout_lineups(url='https://www.fantasyfootballscout.co.uk/team-news/') -> Dict[str, str]:
    """
    Scrapes the latest predicted lineups from Fantasy Football Scout

    :param url: The url for where the predicted lineups are located
    :return: A dictionary of the format {team1: [player1, player2... player11], ... team20: [...] }
    """

    # scrape the raw HTML and parse it
    html = get(url).text
    ...
    final_dictionary = create_final_dictionary(lineups_data=lineup_dictionary, mapping_dictionary=team_names_mapper)
